...
void CDialog_Try::OnBnClickedButton2()
{
	...
	if (bCrypto)
	{
		...
		nMsgLen = st_JsonRoot.toStyledString().length();
		Cryption_XCrypto_Encoder((LPCXBTR)st_JsonRoot.toStyledString().c_str(), &nMsgLen, (UCHAR*)tszMsgBuffer, W2A(tszPassBuffer));
		APIClient_Http_Request(_X("POST"), tszUrlAddr, tszMsgBuffer, NULL, &ptszMsgBuffer, &nMsgLen);
	}
	else
	{
...
void CDialog_Try::OnBnClickedButton3()
{
	...
	st_JsonObject["enVMode"] = (ENUM_VERIFICATION_MODULE_SERIAL_TYPE)m_ComboType.GetCurSel();
