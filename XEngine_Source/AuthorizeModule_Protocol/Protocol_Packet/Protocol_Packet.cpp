...
bool CProtocol_Packet::Protocol_Packet_HttpClientInfo(XCHAR* ptszMsgBuffer, int* pInt_MsgLen, AUTHREG_USERTABLE* pSt_UserTable)
{
	...
	st_JsonUser["xhToken"] = (Json::Value::Int64)pSt_UserTable->st_UserInfo.xhToken;
	...
	st_JsonUser["nTimeCount"] = (Json::Value::Int64)pSt_UserTable->nTimeCount;
...
bool CProtocol_Packet::Protocol_Packet_HttpClientList(XCHAR* ptszMsgBuffer, int* pInt_MsgLen, AUTHSESSION_NETCLIENT*** pppSt_OnClient, int nOnCount, AUTHREG_USERTABLE*** pppSt_OffClient, int nOffCount)
{
	...
	for (int i = 0; i < nOnCount; i++)
	{
		...
		st_JsonUser["xhToken"] = (Json::Value::Int64)(*pppSt_OnClient)[i]->st_UserTable.st_UserInfo.xhToken;
		...
		st_JsonUser["nTimeCount"] = (Json::Value::Int64)(*pppSt_OnClient)[i]->st_UserTable.nTimeCount;
	...
	for (int i = 0; i < nOffCount; i++)
	{
		...
		st_JsonUser["xhToken"] = (Json::Value::Int64)(*pppSt_OffClient)[i]->st_UserInfo.xhToken;
		...
		st_JsonUser["nTimeCount"] = (Json::Value::Int64)(*pppSt_OffClient)[i]->nTimeCount;
		st_JsonUser["nUserState"] = 0;
...
bool CProtocol_Packet::Protocol_Packet_HttpTryList(XCHAR* ptszMsgBuffer, int* pInt_MsgLen, AUTHREG_TEMPVER*** pppSt_TryList, int nListCount)
{
	...
	for (int i = 0; i < nListCount; i++)
	{
		...
		st_JsonObject["enVMode"] = (*pppSt_TryList)[i]->enSerialType;
		if (ENUM_VERIFICATION_MODULE_SERIAL_TYPE_DAY == (*pppSt_TryList)[i]->enSerialType)
		{
...
备注：
*********************************************************************/
bool CProtocol_Packet::Protocol_Packet_HttpOAuth(XCHAR* ptszMsgBuffer, int* pInt_MsgLen, AUTHREG_OAUTHINFO* pSt_OAuthInfo)
{
	Protocol_IsErrorOccur = false;
	...
	Json::Value st_JsonRoot;
	Json::Value st_JsonOAuth;

	st_JsonOAuth["tszClientID"] = pSt_OAuthInfo->tszClientID;
	st_JsonOAuth["tszClientKey"] = pSt_OAuthInfo->tszClientKey;
	st_JsonOAuth["tszUserName"] = pSt_OAuthInfo->tszUserName;
	st_JsonOAuth["nExpirationTime"] = pSt_OAuthInfo->nExpirationTime;

	st_JsonRoot["msg"] = "success";
	st_JsonRoot["code"] = 0;
	st_JsonRoot["st_OAuthInfo"] = st_JsonOAuth;
...
备注：
*********************************************************************/
bool CProtocol_Packet::Protocol_Packet_HttpOAuth2(XCHAR* ptszMsgBuffer, int* pInt_MsgLen, AUTHREG_OAUTHINFO* pSt_OAuthInfo)
{
	Protocol_IsErrorOccur = false;
	...
	st_JsonRoot["access_token"] = pSt_OAuthInfo->tszTokenStr;
	st_JsonRoot["refresh_token"] = pSt_OAuthInfo->tszUPToken;
	st_JsonRoot["token_type"] = "Bearer";
	st_JsonRoot["expires_in"] = pSt_OAuthInfo->nExpirationTime;
