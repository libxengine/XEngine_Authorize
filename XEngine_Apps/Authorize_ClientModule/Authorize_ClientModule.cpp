...
//Linux MACOS::g++ -std=c++17 -Wall -g Authorize_ClientModule.cpp -o Authorize_ClientModule.exe -L ../../XEngine_Release/ -lXEngine_Verification -ljsoncpp -Wl,-rpath=../../XEngine_Release

int TestClient_HardLogin()
{
	if (!Verification_XAuthNet_Connect(_X("127.0.0.1"), 5300))
	{
		printf("Verification_XAuthNet_Connect:%lX\n", Verification_GetLastError());
		return -1;
	}
	if (!Verification_XAuthNet_Login("2FDWAD02JD2091", ""))
	{
...
int TestClient_TryLogin()
{
	...
	if (Verification_XAuthNet_TryRequest(lpszTryInsert))
	{
		printf("ok\n");
	}
	else
